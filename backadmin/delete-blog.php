...
if($admin == 1)
{
   if(($_POST))
   {
      ...
      $get_service_query = "select * from blogs where id='$delete_id' and is_deleted='0'";
      ...
      if(count($all_service_data_array) > 0)
      {
         ...
         $update_service_query = "update blogs set is_deleted='1' where id='$delete_id'";
         $result_update_service_query = mysqli_query($db_mysqli,$update_service_query);
         if($result_update_service_query)
         {
            $return["html_message"] = 'Blog Removed Successfully.';
      ...
      {
         $return["html_message"] = 'Blogs Does Not Exist.';
         $return["status"] = "error";
         echo json_encode($return);
         exit();
      }
