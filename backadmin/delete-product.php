...
if($admin == 1)
{
   if(($_POST))
   {
      ...
      $get_product_query = "select * from product where id='$delete_id' and is_deleted='0'";
      ...
      if(count($all_product_data_array) > 0)
      {

         $update_product_query = "update product set is_deleted='1' where id='$delete_id'";
         $result_update_product_query = mysqli_query($db_mysqli,$update_product_query);
         if($result_update_product_query)
         {
            $return["html_message"] = 'Product Removed Successfully.';
