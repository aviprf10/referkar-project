...
if($admin == 1)
{
   if(($_POST))
   {
      ...
      $get_category_query = "select * from sub_category where id='$delete_id' and is_deleted='0'";
      ...
      if(count($all_category_data_array) > 0)
      {
         ...
         $update_category_query = "update sub_category set is_deleted='1' where id='$delete_id'";
         $result_update_category_query = mysqli_query($db_mysqli,$update_category_query);
         if($result_update_category_query)
         {
            $return["html_message"] = 'Sub Category Removed Successfully.';
