...
if ($_SERVER['REQUEST_METHOD'] == 'POST')
{
    ...
    if($disclaimer == 'false')
    {
        $return["message"] = 'Please checked Disclaimer checkbox';
        $return["status"] = "error";
    ...
    $mobileverificationstatus = Secure1($db_mysqli,ucfirst($_POST['mobileverificationstatus']));
    $mobile_verify = 1;
    $email_verify = 1;
    ...
    if(!empty($all_user_data_array))
    {
        ...
        if ($result_insert_loan_inquiry_query)
        {
            ...
            $return["message"] = 'Inquiry Send Successfully! i will get back touch you.';
        ...
        {
            $return["message"] = 'Some Error Occured While adding Inquiry';
            $return["status"] = "error";
            $return["status_code"] = 100;
            echo json_encode($return);
            exit();
    ...
    else 
    {
        ...
        {
            $return["message"] = 'Please Verified mobile number.';
            $return["status"] = "error";
            $return["status_code"] = 100;
            echo json_encode($return);
            exit();
