...
if ($admin == 1)
{
    ...
    $get_category_query = "select sc.*, c.category_name from sub_category sc LEFT JOIN category c on sc.category_id = c.id where $custom_filter $order_by LIMIT $limit_start,$limit_end";
    $result_get_category_query = mysqli_query($db_mysqli, $get_category_query);

    $count_get_category_query = "select * from sub_category sc LEFT JOIN category c on sc.category_id = c.id where $custom_filter";
    ...
    if (count($all_category_data_array) > 0)
    {
        foreach ($all_category_data_array as $all_category_table_data)
        {
            ...
            $nestedData[] = '
                <center>
                    <a href="' . $base_url . 'edit-sub-category/' . $row_id . '" title="Edit" class="tooltip_class" data-popup="tooltip"><i class="icon-pencil5 text-primary"></i></a>
                    &nbsp;&nbsp;
