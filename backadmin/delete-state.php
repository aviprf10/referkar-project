...
if ($admin == 1 || ($sub_admin == 1 && $is_state_write == 1))
{
    if (($_POST))
    {
        ...
        $get_state_query = "select * from states where id IN ($delete_id_list) and is_deleted='0'";
        ...
        {
            $update_state_query = "UPDATE states
                                      LEFT JOIN cities ON states.id = cities.state_id
                                    SET states.is_deleted = 1, cities.is_deleted = 1
                                    WHERE states.id IN ($delete_id_list)";
            $result_update_state_query = mysqli_query($db_mysqli, $update_state_query);
            if ($result_update_state_query)
            {
                $return["html_message"] = 'State Removed Successfully.';
